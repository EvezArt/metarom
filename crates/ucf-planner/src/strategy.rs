...
pub struct ScoreWeights {
    pub fidelity: u16, pub latency: u16,
    pub engineering_effort_inverted: u16, pub runtime_cost_inverted: u16,
    pub legal_risk_inverted: u16, pub determinism: u16,
    pub user_friction_inverted: u16,
}

impl Default for ScoreWeights {
    fn default() -> Self {
        Self { fidelity: 22, latency: 20, engineering_effort_inverted: 12,
...
impl WorkingAxes {
    ...
    fn to_plan_scores(self, weights: ScoreWeights) -> PlanScores {
        let mut s = PlanScores {
            ...
            engineering_effort: clamp(self.engineering_effort), runtime_cost: clamp(self.runtime_cost),
            legal_risk: clamp(self.legal_risk), determinism: clamp(self.determinism),
            user_friction: clamp(self.user_friction), total: 0,
        };
        let total_i =
        ...
        let max_total = 100 * (weights.fidelity as i32 + weights.latency as i32 +
            ...
            weights.legal_risk_inverted as i32 + weights.determinism as i32 + weights.user_friction_inverted as i32);
...
pub fn score_strategy(
    ...
    match strategy {
        ...
        Strategy::StreamingRecommended => {
            a.fidelity -= 10; a.latency -= 25; a.runtime_cost -= 10; a.determinism -= 20; a.user_friction -= 10;
            if helper_present { a.latency += 5; }
        }
        Strategy::SplitExecutionRecommended => {
            a.fidelity -= 5; a.latency -= 15; a.engineering_effort -= 25; a.runtime_cost -= 15; a.determinism -= 15; a.user_friction -= 10;
            if helper_present { a.latency += 8; }
        }
        Strategy::AugmentationRequired => { a.engineering_effort -= 30; a.runtime_cost -= 20; a.user_friction -= 25; }
        Strategy::NotFeasible => { a = WorkingAxes { fidelity: 0, latency: 0, engineering_effort: 100, runtime_cost: 100, legal_risk: 100, determinism: 0, user_friction: 100 }; }
    }
    if policy.prefer_local_execution {
        match strategy {
            Strategy::StreamingRecommended | Strategy::SplitExecutionRecommended => { a.latency -= 5; a.user_friction += 5; }
            _ => {}
        }
    }
